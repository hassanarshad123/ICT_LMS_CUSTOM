...
cd "$BACKEND_DIR"
docker build -t "lms-backend:$GIT_SHA" -t "lms-backend:latest" .
...
docker compose up -d "lms-$DEPLOY_SLOT"
...
if [ -n "$NEW_MIGRATIONS" ]; then
    ...
    if [ "$DANGEROUS" = "true" ] && [ "${FORCE_MIGRATE:-}" != "true" ]; then
        docker compose stop "lms-$DEPLOY_SLOT"
...
if [ "$HEALTHY" != "true" ]; then
    ...
    docker compose logs "lms-$DEPLOY_SLOT" >> "$DEPLOY_LOG" 2>&1 || true
    docker compose stop "lms-$DEPLOY_SLOT"
...
if [ "$ACTIVE_SLOT" != "none" ]; then
    ...
    sleep "$DRAIN_SECONDS"
    docker compose stop "lms-$ACTIVE_SLOT" 2>/dev/null || true
