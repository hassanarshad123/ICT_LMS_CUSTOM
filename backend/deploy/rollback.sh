...
if docker ps -a --format '{{.Names}}' | grep -q "lms-$ROLLBACK_SLOT"; then
    docker start "lms-$ROLLBACK_SLOT" 2>/dev/null || docker compose -f deploy/docker-compose.yml up -d "lms-$ROLLBACK_SLOT"
else
    log "Container lms-$ROLLBACK_SLOT not found — starting from compose..."
    docker compose -f deploy/docker-compose.yml up -d "lms-$ROLLBACK_SLOT"
fi
...
docker compose -f deploy/docker-compose.yml stop "lms-$CURRENT_SLOT" 2>/dev/null || true
