...
async def get_access_token(
    ...
    *,
    ...
    Raises:
        PayFastAuthError: non-2xx status or response missing ``ACCESS_TOKEN``.
        PayFastError:     network / timeout errors.
    """
    ...
    try:
        response = await http_client.post(  # type: ignore[union-attr]
            url,
            data=form_data,
            headers={"User-Agent": "Zensbot-LMS/1.0 PayFast"},
        )
    except httpx.TimeoutException as exc:
        raise PayFastError(
            f"Request to PayFast token endpoint timed out: {exc}"
        ) from exc
    except httpx.HTTPError as exc:
        raise PayFastError(
            f"HTTP error communicating with PayFast: {exc}"
        ) from exc
    finally:
        if _owns_client:
