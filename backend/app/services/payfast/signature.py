...
Per PayFast's IPN Integration Document (Apr 2026), PayFast authenticates IPN
callbacks with a ``validation_hash`` field IN the body:
...
There is no header-based HMAC. The ``secured_key`` used for GetAccessToken is
the same key used for IPN validation.
"""
