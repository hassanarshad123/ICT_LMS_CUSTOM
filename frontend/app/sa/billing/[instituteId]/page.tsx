      ...
      <div className="bg-white rounded-2xl p-5 border border-zinc-200">
        ...
        {!editMode ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div><label className="text-xs text-zinc-500 block mb-1">Base Amount (PKR)</label><input type="number" value={form.baseAmount || ''} min={0} onChange={(e) => setForm({ ...form, baseAmount: Math.max(0, parseInt(e.target.value) || 0) })} className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" /></div>
            ...
            <div><label className="text-xs text-zinc-500 block mb-1">Extra User Rate (PKR)</label><input type="number" value={form.extraUserRate || ''} min={0} onChange={(e) => setForm({ ...form, extraUserRate: Math.max(0, parseInt(e.target.value) || 0) })} className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" /></div>
            <div><label className="text-xs text-zinc-500 block mb-1">Extra Storage Rate (PKR/GB)</label><input type="number" value={form.extraStorageRate || ''} min={0} onChange={(e) => setForm({ ...form, extraStorageRate: Math.max(0, parseInt(e.target.value) || 0) })} className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" /></div>
            <div><label className="text-xs text-zinc-500 block mb-1">Extra Video Rate (PKR/GB)</label><input type="number" value={form.extraVideoRate || ''} min={0} onChange={(e) => setForm({ ...form, extraVideoRate: Math.max(0, parseInt(e.target.value) || 0) })} className="w-full px-3 py-2 border border-zinc-200 rounded-lg text-sm" /></div>
          </div>
        )}
      </div>
