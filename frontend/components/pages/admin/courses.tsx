...
import { BookOpen, Plus, X, Trash2, Edit3, Loader2, Search, ImagePlus } from 'lucide-react';
...
export default function AdminCourses() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (coverFile) {
        try { await uploadCourseCover(course.id, coverFile); } catch { toast.error('Course created but cover upload failed'); }
      }
  ...
  const handleCoverSelect = (file: File) => {
    ...
    if (!['image/png', 'image/jpeg', 'image/webp'].includes(file.type)) { toast.error('Only PNG, JPG, WebP.'); return; }
    setCoverFile(file);
  ...
  return (
    <DashboardLayout>
      ...
      <div className="space-y-3 mb-6">
        ...
        {/* Row 2: Status dropdown (left) + Create button (right) */}
            ...
            <div>
              ...
              {coverPreview ? (
                <div className="relative w-full h-32 rounded-xl overflow-hidden border border-gray-200">
                  ...
                  <button type="button" onClick={() => { setCoverFile(null); setCoverPreview(null); }} className="absolute top-2 right-2 p-1 bg-black/50 hover:bg-black/70 rounded-lg transition-colors">
                ...
                <label className="flex items-center justify-center gap-2 w-full h-24 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-gray-300 transition-colors">
                  ...
                  <input type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) handleCoverSelect(f); }} />
                </label>
              )}
