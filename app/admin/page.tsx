...
import { batches, students, teachers } from '@/lib/mock-data';
import { Layers, Users, GraduationCap, TrendingUp } from 'lucide-react';
import Link from 'next/link';

const stats = [
  { label: 'Total Batches', value: batches.length, icon: <Layers size={22} />, color: 'bg-[#C5D86D]', href: '/admin/batches' },
  { label: 'Total Students', value: students.length, icon: <Users size={22} />, color: 'bg-[#E8E8E8]', href: '/admin/students' },
  { label: 'Total Teachers', value: teachers.length, icon: <GraduationCap size={22} />, color: 'bg-[#C5D86D]', href: '/admin/teachers' },
  { label: 'Active Batches', value: batches.filter((b) => b.status === 'active').length, icon: <TrendingUp size={22} />, color: 'bg-[#E8E8E8]', href: '/admin/batches' },
];
            ...
            {batches.slice(0, 4).map((batch) => (
              <div key={batch.id} className="flex items-center justify-between p-4 rounded-xl bg-gray-50">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Teacher: {batch.teacherName}</p>
                </div>
